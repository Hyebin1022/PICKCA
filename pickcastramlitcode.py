...
if not data.empty:
    ...
    if pick_button and user_input.strip():
        ...
        # If recommendations are fewer than 5, add more
        if len(recommendations) < 5:
            new_recommendations = []
            existing_ids = {x[3] for x in recommendations}  # To avoid duplicates based on cafe ID
            
            # Loop through the data again to find additional recommendations
            for row in data.itertuples(index=False):
                if len(recommendations) >= 5:
                    break  # Stop when we reach 5 recommendations
                
                # Check for duplicates based on ID
                if row.id in existing_ids:
                    continue
                
                # Calculate similarity again for new rows
                similarity = cosine_similarity(
                    np.array(user_topic_dist).reshape(1, -1),
                    np.array(row.degree_of_fit).reshape(1, -1)
                )[0][0]
            ...
            # Sort new recommendations by similarity
            new_recommendations = sorted(new_recommendations, key=lambda x: x[1], reverse=True)
            
            # Merge the two lists and keep the top 5 recommendations
